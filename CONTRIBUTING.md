...
[INDEX.md](./INDEX.md)부터 열어 주세요 — 전체 문서 지도이자, 각 시스템이 무엇을
...
모든 작업에 강제되는 절차가 아닙니다 — 문제 성격에 따라 시작 지점이 달라집니다.
...
4. 개발자가 초안을 검토·수정한 뒤 셀프 머지합니다 — 이 머지가 "의도 확정"의
   승인입니다.
...
요약본도 함께 갱신하시려면, 새 스펙을 기준으로 해당 파일 내용을 다시 생성해
주셔야 합니다 (엔드포인트 태그별 목록입니다).
...
없습니다). 빌드 직전에는 `release/<버전>` 형식의 annotated 태그를 찍어 어느
...
- [ ] 코드 컨벤션(`CLAUDE.md`)을 따랐는지 확인했습니다 — 색상 하드코딩 없이
      `AppColors` 참조, MVVM 계층 분리 등입니다.
